...
bin/hello/bootstrap:
	GOOS=linux GOARCH=amd64 go build -tags lambda.norpc -ldflags="-s -w" -o bin/hello/bootstrap codedeploy/hello

bin/prehook.zip: bin/prehook/bootstrap
	zip -j bin/prehook.zip bin/prehook/bootstrap

bin/prehook/bootstrap:
	GOOS=linux GOARCH=amd64 go build -tags lambda.norpc -ldflags="-s -w" -o bin/prehook/bootstrap codedeploy/prehook
